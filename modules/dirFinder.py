...
def dirFinder():
    ...
    try:
        ...
        max_threads = questionary.text("Enter max concurrent requests [default 500]:").ask()
        try:
            max_threads = int(max_threads) if max_threads else 500
        except ValueError:
            max_threads = 500

        timeout = questionary.text("Enter max timeout for requests [default 2]:").ask()
        try:
        ...
        recursiveExploration = questionary.confirm(
            "Want to use recursive exploration(This may take a while)?", default=True #add in settings the option to set it to true by default, change wordlist used by this, maxdepth; ...
        ).ask()
        ...
        building = False
        asyncio.run(start(target, wordlist, max_threads, aiohttp.ClientTimeout(total=timeout), proxy, recursiveExploration))
...
async def start(target, wordlistPath, max_threads, timeout, proxy, recursiveExploration):
    ...
    async with aiohttp.ClientSession(connector=conn, timeout=timeout) as session:
        ...
        #ADD this after random.shuffle(words)
        ...
        with progress:
            ...
            while recursiveExploration and current_depth <= MAX_DEPTH and recursive_tasks:
                new_wordlist = defaultWordlistDirFinderRecursive if os.path.isfile(os.path.join(os.path.dirname(os.path.abspath(__file__)), "../", defaultWordlistDirFinderRecursive)) else wordlistPath
                with open(new_wordlist, "r", encoding="utf-8") as f:
                    newWords = [w.strip().lstrip("/") for w in f if w.strip()]
                #ADD this after random.shuffle(newWords)

                print(f"[bold blue]🔍 Starting recursive scan with depth {current_depth}...[/bold blue]")
                next_recursive_tasks = []
                for path_parts in recursive_tasks:
                    base_path = "/".join(path_parts)
                    url = f"{validUrl}/{base_path}"

                    progress.update(task1, description=f"Recursive scanning /{base_path}...")
                    progress.update(task1, total=len(newWords), completed=0)
                    coros = [fetch_with_retries(f"{base_path}/{newWord}", session, validUrl, proxy, semaphore) for newWord in newWords]
                    for fut in asyncio.as_completed(coros):
                        url, status, location, err, word = await fut
                        progress.update(task1, advance=1)
                        ...
                        if status != 429:
                            print(f"[cyan]{url}[/cyan] {status_str}" + (f" → [yellow]{location}[/yellow]" if location else ""))
                            if recursiveExploration:
                                next_recursive_tasks.append(path_parts + [word.split('/')[-1]])
...
async def ping(session, url, timeout, proxy):
    ...
    for u in urls:
        try:
            ...
            async with session.get(u, headers={"User-Agent": random.choice(USER_AGENTS)}, ssl=ssl_param, timeout=timeout, proxy=proxy) as resp:
                return True, u
        except:
            continue
    return False, None
