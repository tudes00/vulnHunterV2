...
def combinations(words, symbol):
  ...
  results = [
      symbol.join(words_split[:i + 1]) for i in range(len(words_split))
      if not (i == 0 and len(words_split) == 1)
  ]
...
def concat_extensions(results, extensions):
  ...
  extended_results = [
      ''.join([result, extension]) for result in results
...
def main():
  ...
  parser.add_argument('ext_file',
                      type=str,
                      help='The file path for the extensions')
  parser.add_argument('--full',
                      action='store_true',
                      help='Include full urls with http/https scheme')
